...
public interface FlightRepository extends JpaRepository<Flight, String> {
    ...
    List<Flight> findUpcomingFlightsByRoute(@Param("origin") String origin, @Param("destination") String destination,
    ...
    List<Flight> searchFlights(@Param("origin") String origin, @Param("destination") String destination,
    ...
    List<Flight> findFlightsByRouteWithAvailableSeats(@Param("origin") String origin,
            @Param("destination") String destination);

    // Find flights departing within time window (for reminders)
    @Query("SELECT f FROM Flight f WHERE f.departureTime > :startTime AND f.departureTime <= :endTime AND (f.status = 1 OR f.status = 4) AND f.isDeleted = false ORDER BY f.departureTime ASC")
    List<Flight> findFlightsDepartingBetween(@Param("startTime") LocalDateTime startTime,
