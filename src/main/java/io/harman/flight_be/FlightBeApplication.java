...
public class FlightBeApplication {
	...
	public CommandLineRunner createDummyData(
		...
		return args -> {
			...
			String[] airlineCodes = { "GA", "JT", "ID", "QG", "QZ", "SJ", "IW", "IN", "IU", "TN" };
		...
		for (int i = 0; i < 20; i++) {
			try {
				...
				// Extract airline ID from airplane ID (format: XX-XXX)
				String airlineId = selectedAirplaneId.substring(0, 2);
	...
	System.out.println("Available flight IDs: " + flightIds);
	...
	System.out.println("✓ Created " + classFlightIds.size() + " class flights");			// 5. Create Seats (at least 300 - 5 seats per class flight)
			System.out.println("\n[5/7] Creating Seats...");
			int seatCount = 0;
		...
		for (int i = 0; i < 15; i++) {
			try {
				...
				if (flightIds.isEmpty()) {
					System.err.println("Warning: No flights available for booking creation");
					break;
				}
		...
		System.out.println("✓ Created " + bookingCount + " bookings");			System.out.println("\n" + "=".repeat(60));
			System.out.println("Dummy data generation complete!");
			System.out.println("Summary:");
			System.out.println("  - Airlines: 10");
			System.out.println("  - Airplanes: 15");
			System.out.println("  - Flights: 20");
			System.out.println("  - Class Flights: 60");
