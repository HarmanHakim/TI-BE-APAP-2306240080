...
public class LoyaltyServiceImpl implements LoyaltyService {
    ...
    public PurchasedCouponResponseDTO purchaseCoupon(PurchaseCouponRequestDTO request) {
        Coupon coupon = couponRepository.findById(request.getCouponId())
                .orElseThrow(() -> new IllegalArgumentException("Coupon with id " + request.getCouponId() + " not found"));
        ...
        UserProfileDTO userProfile = externalApiService.getUserProfile(request.getCustomerId());
        String code = generateCouponCode(coupon, userProfile, request.getCustomerId());
    ...
    private String generateCouponCode(Coupon coupon, UserProfileDTO userProfile, UUID customerId) {
        ...
        String userPart = sanitize(resolveUserName(userProfile, customerId));
    ...
    private String resolveUserName(UserProfileDTO userProfile, UUID customerId) {
        if (userProfile == null) {
            return customerId.toString().substring(0, 6).toUpperCase();
        }
